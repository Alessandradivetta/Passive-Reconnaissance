...
Let’s use dig to look up the MX records and compare them to nslookup. We can use dig example.com, but to specify the record type, we would use dig example.com MX. Optionally, we can select the server we want to query using dig @SERVER DOMAIN_NAME TYPE.
